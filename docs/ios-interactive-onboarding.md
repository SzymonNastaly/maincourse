...
`ImportDemoView` owns presentation only. It has no account or network dependency and never sends the preview's synthetic ID to the API. The signup flow and the empty cookbook reuse it. The empty invitation appears only after a successful empty load and can be dismissed per user. Settings does not offer an example replay. The empty cookbook leads with “Let’s start saving recipes,” followed by the import button and sharing instructions as alternative ways to start.
