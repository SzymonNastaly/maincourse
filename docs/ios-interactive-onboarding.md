...
`OnboardingCoordinator` is owned by `RootView`. Only the recipe preview's bottom Sign up action or an authenticated Keep creates a durable request UUID. Anonymous intent binds at the first authentication boundary; an already authenticated Keep binds immediately. Destination binding waits for the existing session owner to resolve the personal cookbook. Retries retain the original source, destination, and UUID. A session change clears bound intent and stale UI state; late results cannot navigate or alter another account.
...
The post keeps Log in in the leading navigation position. After the sharing exercise, Sign up becomes the bottom recipe action; choosing it records the keep intent so successful registration automatically saves the sample. Its share hint appears after three active seconds, stops after sharing, and uses a static bubble with Reduce Motion. The simulated social panel uses fictional contacts; the app panel uses muted placeholders and a horizontal row sized to leave half of MainCourse visible at the trailing edge. These placeholders never send content to another app. The creator avatar is a bundled crop of the owner-provided photo, separate from the cookbook brand mark.
