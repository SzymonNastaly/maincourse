...
Activity 1.13.0 and Navigation 3 align the AndroidX Lifecycle atomic group to
2.10.0 even though the two direct Lifecycle dependencies remain declared at
2.9.4. Do not force one Lifecycle artifact back to 2.9.4: the group must resolve
together. Milestone 1 uses OkHttp/kotlinx.serialization, Room, Android Keystore
storage, and a session-owned Coil image loader.
...
The owner-recorded **Android** OAuth client in #92 uses
